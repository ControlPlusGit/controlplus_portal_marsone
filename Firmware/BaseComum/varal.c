...
const u32 MEU_ID = 0x0000170;
//volatile u32 MEU_ID = 0x0000030;
...
int ContadorDiferencial;
int ContadorDeEcrita;
int ContadorDeLeitura;
...
int DecrementaContador (int Contador);
...
int incrementarContadorLivre (int Contador){
    if (Contador < (TAMANHO_BUFFER_DE_TAGS)){
        Contador = Contador + 1;
    }
...
int IncrementarContador (int Contador, int Limite){
...
int IncrementaContadorLivre (int Contador, int Limite){
    if (Contador < (Limite - 1)){
        Contador = Contador + 1;
    } else {
        Contador = 0;
...
int haElementosNaFifo (void){
    if (ContadorDiferencial != 0){
        return 0;
    }
    return -1;
}
...
int fifoSobreEscrita (void){
    if (ContadorDiferencial > (TAMANHO_BUFFER_DE_TAGS - 1)){
        return 0;
    }
    return -1;
}
...
int fifoCheia (void){
    if (ContadorDiferencial == (TAMANHO_BUFFER_DE_TAGS)){
        return 0;
    }
    return -1;
}
...
/*
int AdicionaElementoAFifo (char *Elemento){
    if (fifoCheia() == 0){
        return -1;
    }

    if (fifoSobreEscrita() == 0){
        return -1;
    }
    
    (void)sprintf((char*)BufferDeTags[ContadorDeEcrita],"%s",Elemento); //LUCIANO: Apliquei um cast para eliminar um warning.
    
    ContadorDeEcrita = IncrementaContadorLivre(ContadorDeEcrita, TAMANHO_BUFFER_DE_TAGS);
    ContadorDiferencial = IncrementarContador (ContadorDiferencial, (TAMANHO_BUFFER_DE_TAGS + 1));
    
    return 0;
}
*/

int ObtemElementoDaFifo (char *Elemento){
    if (fifoVazia() == 0){
        return -1;
    }
    
    (void)sprintf(Elemento, "%s", BufferDeTags[ContadorDeLeitura]);
    memset(BufferDeTags[ContadorDeLeitura], 0, TAMANHO_TAG);

    ContadorDeLeitura = IncrementaContadorLivre(ContadorDeLeitura, TAMANHO_BUFFER_DE_TAGS);
    ContadorDiferencial = DecrementaContador(ContadorDiferencial);
...
void IniciaVaral (void){
    //char rascunho[20];    //LUCIANO: Variavel nao usada. Comentei para eliminar um warning.
    //int contador;         //LUCIANO: Variavel nao usada. Comentei para eliminar um warning.
...
/*
    ...
    //for (contador = 0;contador < 4;contador = contador + 1){
        if (rascunho[contador] < 'A')
            rascunho[contador] = rascunho[contador] - '0';
        else
            rascunho[contador] = rascunho[contador] - 'A' + 10;
        MEU_ID = MEU_ID | rascunho[contador];
        MEU_ID = MEU_ID << 4;
    }

    MEU_ID = MEU_ID & 0x0000FFFF;
    */    
}
...
void EnviaByteParaWifi (char dado){
    U3TXREG = dado;
...
int RespondeADescobertaNoVaral (unsigned long meuid, long destino){
    ...
    chksum_gerado = GeraCheckSum(chksum_gerado, pacote, 14);    //LUCIANO: Passando o endereco da string 'pacote'. Eliminei um warning.
    ...
    return 0;   //LUCIANO: Adicionei um reteurn para eliminar um warning.
}
...
void EnviaBufferDeTagsAoPCPorEth_bloqueia (void){
    ...
    for (contador = 0;contador < TAMANHO_BUFFER_DE_TAGS;contador = contador + 1){
        if (posicaoOcupadaNoBufferDeTags(contador) != -1){
            ...
            sprintf(rascunho,"00100,%s\r\n",BufferDeTags[contador]);
#ifndef OPERACAO_POR_WIFI
            EnviaStringPara485(rascunho);
#else
            EnviaStringParaWifi(rascunho);
#endif
    ...
    */
    ...
    for (contador = 0;contador < TAMANHO_BUFFER_DE_TAGS;contador = contador + 1){
        if (posicaoOcupadaNoBufferDeTags(contador) != -1){
            ...
            EnviaPacoteParaSerial((char*)BufferDeTags[contador], TAMANHO_EPC);
            eliminaTagDoBuffer(contador);
...
int processaEHT (void){
    EnviaBufferDeTagsAoPC_bloqueia();
    return 0;   //LUCIANO: Adicionei um reteurn para eliminar um warning.
}
...
void CabecaCUP (u32 destino, int tamanho){
    ...
    */
    ...
    chksum_gerado = ResetaCheckSum();
    chksum_gerado = GeraCheckSum(chksum_gerado, pacote, 14);    //LUCIANO: Apenas passando corretamente a string pacote. Eliminei um warning.
...
void ProcessaTagsParaOTrator (int antena){
    
}

void ProcessaTagsParaOEth (int antena){
    char rascunho[200]; //LUCIANO: Retirei o atributo '__attribute__((far))'. Eliminei um warning.
    int contador;
    ...
    for (contador_de_tags = 0; contador_de_tags < MAXTAG;contador_de_tags = contador_de_tags + 1){
        ...
        if (resultado != 0){ // Tem tag na posicao?
            rascunho[0] = '0';
            rascunho[0] = rascunho[0] + antena;
            rascunho[1] = ',';

            tamanho = tags_[contador_de_tags].epclen;
            for (contador = 0;contador < tamanho;contador = contador + 1){
                sprintf(&rascunho[2 + (contador * 2)], "%02X", tags_[contador_de_tags].epc[contador]);
            }
            ...
            for (contador = 0;contador < (TAMANHO_BUFFER_DE_TAGS - 1 );contador = contador + 1) {
                if ( tagJaPresenteNoBuffer ((unsigned char*)rascunho, contador, strlen(rascunho) ) == 0){   //LUCIANO: Apliquei um cast. Eliminei um warning.
            ...
            if (resultado == 0){
                ...
                if (contador != -1){
                    adicionarTagAoBuffer(contador, (unsigned char*)rascunho, strlen(rascunho)); //LUCIANO: Apliquei um cast. Eliminei um warning.
                }
            }
        }
    }    
...
void ProcessaTagsParaOVaral (int antena){
    ...
    for (contador_de_tags = 0; contador_de_tags < MAXTAG;contador_de_tags = contador_de_tags + 1){
        ...
        tamanho = tags_[contador_de_tags].epclen;   //LUCIANO: Carreguei o valor da variavel tamanho. Eliminei um warning (e um possivel erro).
        if (resultado != 0){ // Tem tag na posicao?
            for (contador = 0;contador < tamanho;contador = contador + 1){
    ...
    sprintf(rascunho,"%d, %s",antena,tags_[contador_de_tags].epc); //GILSON, linha quebrada, conferir com os codigos antigos
    ...
    for (contador_de_tags = 0; contador_de_tags < MAXTAG;contador_de_tags = contador_de_tags + 1){
        ...
        if (resultado != 0){ // Tem tag na posicao?
            ...
            for (contador = 0;contador < (TAMANHO_BUFFER_DE_TAGS - 1 );contador = contador + 1) {
                ...
                if ( tagJaPresenteNoBuffer ((unsigned char*)rascunho, contador, tamanho ) == 0){    //LUCIANO: Apliquei um cast. Eliminei um warning.
                    resultado = resultado | 1;
                }
            ...
            if (resultado == 0){
                contador = posicaoLivreNoBufferDeTags();
                if (contador != -1){
                    adicionarTagAoBuffer(contador, (unsigned char*)rascunho, tamanho ); //LUCIANO: Apliquei um cast. Eliminei um warning.
...
int processaEth(){
    ...
    if (tamanho > 3){
        uartRxNBytes((u8*)resposta, (u16*)&tamanho);    //LUCIANO: Apliquei castas para eliminacao de warnings.
    }
    
    if (strstr(resposta, "ReadTags") != NULL){
        EnviaBufferDeTagsAoPCPorEth_bloqueia();
    }
    return 0;
}

void processaWifi (void){
    EnviaBufferDeTagsAoPCPorEth_bloqueia();
}
   ...
   switch (EstadoDeRecepcaoDaMaquinaCUP){
       case CABECA_1:
           if (dado == 0x43){
               ...
               chksum_gerado = ResetaCheckSum();
               chksum_gerado = GeraCheckSum(chksum_gerado, (char*)&dado, 1);
               _LATA2 = 0;
           }
...
int CUPComandoEnviarTagsPresentes (int estado, int dado){
    //int cmp;  //LUCIANO: Variavel sem uso. Comentei para eliminar um warning.
...
void EnviaCRC (void){
    ...
    EnviaPacoteParaSerial((char*)data,2);
...
int adicionarTagAoBufferEth (int posicao, unsigned char * dados, int antena, int tamanho){
    int contador;
    if (posicao > TAMANHO_BUFFER_DE_TAGS)
        return -1;
    
    sprintf((char*)BufferDeTags[posicao], "%s",dados);    //LUCIANO: Apliquei um cast para eliminar um warning.
...
#ifdef RF_CONTROL
    AdicionaElementoAFifo((char*)dados);    //LUCIANO: Apliquei um cast para eliminar um warning.
    return 0;
...
void EnviaBufferDeTagsAoPC_PARA_TESTE (void){
    if (ContadorDeTX == 0){
        ...
        EnviaPacoteParaSerial((char*)BufferDeTags[0], TAMANHO_EPC); //LUCIANO: Apliquei um cast para eliminar um warning.
...
void EnviaBufferDeTagsAoPC (void){
    if (ContadorDeTX == 0){
        ...
        */
        if (posicaoOcupadaNoBufferDeTags(contadorDeTagProcesada) != -1){
           //EnviaBytePara485(TAMANHO_EPC);
            EnviaPacoteParaSerial((char*)BufferDeTags[contadorDeTagProcesada], TAMANHO_EPC);
            eliminaTagDoBuffer(contadorDeTagProcesada);
            contadorDeTagProcesada = contadorDeTagProcesada + 1;
        } else {
