...
void aguardoPrimeiraConexaoTCP(void){
    ...
    enviaDadosParaUSBserial(mensagem, strlen(mensagem));
    //ipRemotoPrincipal = {"104.154.047.107"};
    ...
    if(statusDeConexaoTCP == CONNECTED){
        ...
        for(EPC1 = 0; EPC1 < 3; EPC1 = EPC1 + 1){
            ...
            for(EPC2 = 0; EPC2 < 50; EPC2 = EPC2 + 1){
                sprintf(mensagem, "<FF;%c%c%c%c;E;%02d%02d%02d00;%02d%02d20%02d;%02d%02d%02d>", idDoLeitor[0], idDoLeitor[1], idDoLeitor[2], idDoLeitor[3], EPC0, EPC1, EPC2, dataHora->tm_mday, dataHora->tm_mon + 1, dataHora->tm_year - 100, dataHora->tm_hour, dataHora->tm_min, dataHora->tm_sec);
                enviaDadosParaEthPortais(mensagem, strlen(mensagem));
                delay_ms(200);
            }
        }
